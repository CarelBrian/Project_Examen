        ...
        # Application conçu dans le cadre du Devoir DevOps
        ...
        Ce projet est géré par un script Python (`github_project_setup.py`) qui automatise la création d'une arborescence pour un projet GitHub. 
        Voici une brève explication de certaines parties clés du code :
        ...
        La classe `GitHubProject` est responsable de l'interaction avec l'API GitHub. Elle comprend des méthodes pour créer des répertoires, 
        des fichiers, effectuer des commits et créer des tickets. Ces fonctions sont les suivantes:

        - __init__(self, username, access_token, repo_name): Initialise la classe avec les informations nécessaires pour interagir avec l'API GitHub.

        - create_directory(self, path): Crée un répertoire.

        - create_file(self, path, content=""): Crée un fichier avec un contenu donné.

        - git_commit(self, directory, message): Effectue un commit Git dans un répertoire donné.

        - create_issue(self, title, body): Crée un ticket GitHub avec un titre et un corps donnés.


        ### `ProjectSetup` Class

        La classe `ProjectSetup` orchestre le processus de configuration et de création du projet. Elle crée les répertoires nécessaires, 
        génère le contenu du README.md, effectue des commits et crée des tickets. Ces fonctions sont les suivantes:

        - __init__(self, project, directory_name): Initialise la classe avec une instance de GitHubProject et le nom du répertoire.

        - setup_directories(self): Crée la structure de répertoire spécifiée.

        - setup_readme(self): Crée le contenu du fichier README.md.

        - setup_repo(self): Initialise un dépôt Git et effectue un commit initial.

        - create_tickets(self): Crée des tickets GitHub pour différentes tâches.
